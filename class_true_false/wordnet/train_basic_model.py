...
def load_data(train_filename, word_ind_filename, batch_size=64):
    ...
    with open(train_filename) as train_file:
        ...
        for i, line in enumerate(train_reader):
            ...
            # calculate the path similarity of the words in the caption to the object word
            o = wordnet.synsets(nltk.word_tokenize(obj)[0])
            dist_vals = [.0 for w in nltk.word_tokenize(caption)]
            if len(o) > 0:
                o = o[0]
                dists_val = max([wordnet.synsets(w)[0].path_similarity(o) if len(wordnet.synsets(w))>0 and wordnet.synsets(w)[0].path_similarity(o) != None else 0. for w in nltk.word_tokenize(caption)])
            dists_val = torch.tensor(dists_val)
            dists.append(dists_val) 
    ...
    dists_batched = []
    while dists != []:
        dists_batched.append(dists[:batch_size])
        dists = dists[batch_size:]
    #return inputs_batched, labels_batched
    return caps_batched, objs_batched, labels_batched, dists_batched


caps, objs, labels, dists = load_data("../../data/bert_classify_thereis_5caps_seed0/train.tsv",
...
with open("../../data/bert_classify_thereis_5caps_seed0/lstm_preprocessed_train.pkl", "wb") as processed_file:
    pickle.dump((caps, objs, labels, dists), processed_file)
with open("../../data/bert_classify_thereis_5caps_seed0/lstm_preprocessed_train.pkl", "rb") as processed_file:
    caps, objs, labels, dists = pickle.load(processed_file)
...
for epoch in range(NUM_EPOCHS):
    ...
    i = 0
    for (cap_batch, obj_batch, dists_batch), label_batch in list(zip(zip(caps, objs, dists),labels)):
        ...
        preds = model(cap_batch, obj_batch, dists_batch)
...
torch.save(model.state_dict(), "../../logs/base_lstm_classification/models/base_2019-08-02-2lin.pt")
